...
class Shift < ApplicationRecord
  ...
  def break_end_is_after_break_start
    ...
    errors.add(:break_end, 'は休憩開始時間より前にしてください') if break_end < break_start
  end
  def end_time_is_after_break_end
    return if end_time.blank? || break_end.blank?
    errors.add(:end_time, 'は休憩終了時間より前にしてください') if end_time < break_end
  end
  def break_start_is_after_start_time
    return if break_start.blank? || start_time.blank?
    errors.add(:break_start, 'は出勤時間より前にしてください') if break_start < start_time
  end
end
